...
for (const registryCountry of registry.countries ?? []) {
  ...
  if (!TIER_A_CODES.has(code)) {
    ...
    if (promoted) {
      ...
      assert(
        usesSharedAuthorityPacket
          ? profile.completeness?.political_actor_count >= 10
          : profile.completeness?.political_actor_count === 20,
        `${label}: promoted actor roster is incomplete`,
      );
    }
    continue;
  }
